...
#define CACHE_HIGHLY_ASSOC(cp)	((cp)->assoc > 4)
        ...
        /* MODIFICATION */
/* ######################################## */
/* dynamic initiallizing the epsilon */
extern int epsilon; /* means e = 1/32 */ 
...
enum cache_policy {
  ...
  BIP,    /* LRU insetion policy with e probability to go to MRU */
  Dyn     /* Dynamic insertion policy, combines BIP and original LRU */
};

 enum dedicated_set_selection_type{
  lru,
  bip,
  follower
 };

//typedef enum dedicated_set_selection_type dedicated_set_selection_type;
...
struct cache_set_t
{
  struct cache_blk_t **hash;	/* hash table: for fast access w/assoc, NULL for low-assoc caches */
  struct cache_blk_t *way_head;	/* head of way list */
  struct cache_blk_t *way_tail;	/* tail pf way list */
  struct cache_blk_t *blks;	    /* cache blocks, allocated sequentially, so this pointer can also be used for random access to cache blocks */
            /* MODIFICATION */
 enum dedicated_set_selection_type dedicated_set_selection_type;
...
/*################################################################################*/
       /* MODIFICATIONS FOR DYNAMIC CACHE REPLACEMENT */
       int epsilon;      /* epsilon for the percentage of replacement */
       int PSEL_COUNTER; /* follower policy depends on the on the PSEL_counter sign */
       int PSEL_BITS;    /* depending on the bits we compare the LSB with MSB */
       int BIP_CTR;      /* if BIP_COUNTER == 0 incoming line goes to MRU */
                         /* similar to 'e' == 1/32 or 1/64*/
       int BIP_BITS;     /* counter length is the log(e) */
