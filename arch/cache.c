...
/* ########################################################################################## */
  ...
  cp->epsilon = 32;					        // means e = 1/32 // 
  cp->PSEL_COUNTER = (cp->nsets/2); 	    // dynamically initializing counter, based on the #sets 
  cp->PSEL_BITS = (int) (log2(cp->nsets));  // dynamically counting the bits.
  cp->BIP_CTR = 0;			 		   		// increments for every miss, overflows at 32. 
  cp->BIP_BITS = (int) (log2(cp->epsilon)); // epsilon is in cache.h
  ...
  if (!cp->data)
	...
	int sets_in_each_constinuency = (cp->nsets)/64 ; //nset 0 -> 2047
   /* slice up the data blocks */
  for (bindex=0,i=0; i<cp->nsets; i++)
    {
    	
	  /* first set is always in LRU Policy */ 
      if( i % (sets_in_each_constinuency + 1) == 0 )
      	cp->sets[i].dedicated_set_selection_type = lru;
  /* to avoid conflicts we calculate the next set. */
  /* because if i was 0 it would enter in both if statements. */
  /* and the 1st set would start with bip policy. */

      else if( (i+1) % (sets_in_each_constinuency - 1) == 0)
      	cp->sets[i].dedicated_set_selection_type = bip;

 /* uses the "set dueling policy" winner policy */
      else 
      	cp->sets[i].dedicated_set_selection_type = follower;
...
//##########################################################//
//calculate the cache latency of access in cycles.
 ...
 case BIP:
  {
  	...
  	repl = cp->sets[set].way_tail;
  	if(cp->BIP_CTR == 0)
  		update_way_list(&cp->sets[set], repl, Head); // 1/32 place the set in MRU
  	else
  		update_way_list(&cp->sets[set], repl, Tail); // 31/32 place the se in LRU

 	 if(cp->BIP_CTR < cp->epsilon)
  		cp->BIP_CTR++;        //until 32
  	else 
  		cp->BIP_CTR = 0;	  //at 32 we overflow the counter and start again for the start
  break;
  }
  ...
  case LIP:
  {
  	   fprintf(stderr, "%s\n", cp->name );
       repl = cp->sets[set].way_tail;
       update_way_list(&cp->sets[set], repl, Tail); // place at the LRU position
       break;
  }
  ...
  case Dyn:
    {
  	...
  	if(cp->sets[set].dedicated_set_selection_type == lru)
  	{
  		update_way_list(&cp->sets[set], repl, Head);
  		if(cp->PSEL_COUNTER < cp->nsets)
  			cp->PSEL_COUNTER++;
  	}

  	else if(cp->sets[set].dedicated_set_selection_type == bip)
  	{
  		if(cp->BIP_CTR == 0)
  			update_way_list(&cp->sets[set], repl, Head);
  		else
  	...
  	{
  		int PSEL_COUNTER_MSB = (cp->PSEL_COUNTER >> (cp->PSEL_BITS - 1));
  		//If PSEL_COUNTER_MSB is 0 then we use original LRU.
  		if(PSEL_COUNTER_MSB == 0) 
  			update_way_list(&cp->sets[set], repl, Head);
  		else 
  		{
  			if(cp->BIP_CTR == 0)
  			...
  			if (cp->BIP_CTR < cp->epsilon)
  				...
  				cp->BIP_CTR = 0; //overflow and start again from the start.  				
  		}
